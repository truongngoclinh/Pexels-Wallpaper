...
//            if (isContainInvalidTags(detailLink)) {
//                continue;
//            }
            ...
            if (isContainSensitiveWord(name)) {
                continue;
            }
    ...
    private static boolean isContainSensitiveWord(String name) {
        String[] word1 = {"bik"};
        String[] word2 = {"ini"};
        return Pattern.compile(Pattern.quote("woman"), Pattern.CASE_INSENSITIVE).matcher(name).find() ||
                Pattern.compile(Pattern.quote(word1[0] + word2[0]), Pattern.CASE_INSENSITIVE).matcher(name).find();

    }

    private static boolean isContainInvalidTags(String detailLink) throws Exception {
        ArrayList<String> tags = getTagListFromUrl(detailLink);
//        for (String tag : tags) {
//            for (String violentTag : Constants.VIOLENT_TAGS) {
//                if (Pattern.compile(Pattern.quote(violentTag), Pattern.CASE_INSENSITIVE).matcher(tag).find()) {
//                    return true;
//                }
//            }
//        }

        for (int i = 0; i < Constants.VIOLENT_TAGS1.length; i++) {
            if (tags.contains(Constants.VIOLENT_TAGS1[i] + Constants.VIOLENT_TAGS2[i])) {
                return true;
            }
        }
    ...
    public static Observable<ArrayList<Image>> getImageListStartAt(final String mainLink, final int page) {
        ...
        return Observable.zip(obs1, obs2, obs3,
                              new Func3<ArrayList<Image>, ArrayList<Image>, ArrayList<Image>, ArrayList<Image>>() {
                                  ...
                                  public ArrayList<Image> call(ArrayList<Image> o, ArrayList<Image> o2,
                                      ...
                                      o.addAll(o2);
                                      o.addAll(o3);
                                      Timber.e("getImageListStartAt : mainLink = " + mainLink +"?page="+ page);
                                      Timber.e("loading time = " + (System.currentTimeMillis() - startTime));
    ...
    private static ArrayList<String> getTagListFromUrl(String url) throws Exception {
        Document doc = Jsoup.connect(url).timeout(CONNECT_TIMEOUT).get();
        ArrayList<String> tags = new ArrayList<>();
            ...
            public void call(Subscriber<? super List<Category>> subscriber) {
                ...
                try {
                    ...
                    for (Element element : elements) {
                        ...
                        if (Pattern.compile(Pattern.quote(word1[0] + word2[0]), Pattern.CASE_INSENSITIVE).matcher(name)
                                .find() || Pattern.compile(Pattern.quote(word1[1] + word2[1]), Pattern.CASE_INSENSITIVE)
                                .matcher(name).find()) {
                            continue;
                        }
    ...
    public static Observable<Boolean> hasNewVersion(final Context context) {
        return Observable.create(new Observable.OnSubscribe<Boolean>() {
            @Override
            public void call(Subscriber<? super Boolean> subscriber) {
                try {
                    subscriber.onNext(checkVersion(context));
                    subscriber.onCompleted();
                } catch (Exception e) {
                    subscriber.onError(e);
                }
    ...
    private static boolean checkVersion(Context context) throws Exception {
        PackageInfo pInfo = context.getPackageManager().getPackageInfo(context.getPackageName(), 0);
        String versionName = pInfo.versionName;

        String storeVersion = getVersionFromStore(context);

        return !versionName.equalsIgnoreCase(storeVersion);
    }

    private static String getVersionFromStore(Context context) throws Exception {
        ...
        Timber.e("Version name = " + newVersion);
        return newVersion;
    }
}
